...
class FileSearchEngine(ttk.Frame):
    ...
    def __init__(self, master):
        ...
        self.progressbar = ttk.Progressbar(
            master=self, mode=INDETERMINATE, bootstyle=(STRIPED, SUCCESS)
        )
    ...
    def create_type_row(self):
        ...
        contains_opt = ttk.Radiobutton(
            master=type_row, text="Contains", variable=self.type_var, value="contains"
        )
        ...
        endswith_opt = ttk.Radiobutton(
            master=type_row, text="EndsWith", variable=self.type_var, value="endswith"
        )
        endswith_opt.pack(side=LEFT)
        endswith_opt.invoke()

    def create_results_view(self, coldata=[], rowdata=[]):
        ...
        self.resultview.column(
            column=0, anchor=W, width=utility.scale_size(self, 125), stretch=False
        )
        self.resultview.column(
            column=1, anchor=W, width=utility.scale_size(self, 140), stretch=False
        )
        self.resultview.column(
            column=2, anchor=E, width=utility.scale_size(self, 50), stretch=False
        )
        self.resultview.column(
            column=3, anchor=E, width=utility.scale_size(self, 50), stretch=False
        )
        self.resultview.column(column=4, anchor=W, width=utility.scale_size(self, 300))
        ...
        self.resultview1.build_table_data(coldata, rowdata)
        # self.resultview1.get_column(0).configure(stretch=False)
        # self.resultview1.get_column(1).configure(stretch=True)
        # self.resultview1.get_column(1).configure(stretch=True)
        # self.resultview1.get_column(1).configure(stretch=True)
        self.resultview1.get_column(-1).configure(stretch=True)
        self.resultview1.pack(fill=BOTH, expand=YES, pady=10)
    ...
    def check_queue(self, iid):
        ...
        if all([FileSearchEngine.searching, not FileSearchEngine.queue.empty()]):
            filename = FileSearchEngine.queue.get()
            self.insert_row(filename, iid)
            self.update_idletasks()
            self.after(100, lambda: self.check_queue(iid))
        elif all([not FileSearchEngine.searching, not FileSearchEngine.queue.empty()]):
            while not FileSearchEngine.queue.empty():
                filename = FileSearchEngine.queue.get()
                self.insert_row(filename, iid)
            self.update_idletasks()
            self.progressbar.stop()
        elif all([FileSearchEngine.searching, FileSearchEngine.queue.empty()]):
            self.after(100, lambda: self.check_queue(iid))
        else:
            self.progressbar.stop()
    ...
    def insert_row(self, file, iid):
        ...
        try:
            ...
            # self.resultview1.insert_row(1, (_name, _modified, _type, _size, _path))
            self.resultview1.build_table_data(c, r)
            self.resultview1.load_table_data(clear_filters=True)
        except OSError:
            return
    ...
    @staticmethod
    def get_dataframe():
        df = pd.read_excel(io="Book1.xlsx", sheet_name="Sheet1").dropna(
            axis=1, how="all"
        )
        df = df[3:]
